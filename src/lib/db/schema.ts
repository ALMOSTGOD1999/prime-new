...
export const income = pgTable("income", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  type: text("type", { enum: ["direct", "matching", "award", "cashback", "daily_activation", "referral", "performance_incentive", "level"] }).notNull(),
...
export const purchases = pgTable("purchases", {
  ...
  cgst: real("cgst").default(0).notNull(),
  sgst: real("sgst").default(0).notNull(),
  additionalCharges: real("additional_charges").default(0).notNull(),
